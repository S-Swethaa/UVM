...
// uvm_tlm_fifo which is used to point to pont producer to consumer sending the transaction we need to temporarly store the data using tls_fifo inbuilt we call this uvm_tlm_fifo inbuilt create a fifo user defined bounded depth
// we can give the user defined fifo depth here
/*steps 1:create a transaction objects
step 2: create a producer component class
step 3: create a consumer component class
step 4: create a environment create a handle for both create a component and connect the component using connect phase and 
using the uvm_tlm_fifo create a object for this also and define the depth here
step 5 : create a test and start the test vie the console producer and consumer transaction objcet using tlm ports
u can check the fifo has space to put the data using nonblocking try_put ,can_put,fifo has data is there to checking using nonblocking try_get and can_get,and peek method*/
